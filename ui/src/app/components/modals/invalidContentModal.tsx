    ...
    private errorDetail(): React.ReactElement {
        if (this.props.error) {
            if (this.props.error.name === "RuleViolationException" && this.props.error.causes != null && this.props.error.causes.length > 0 ) {
                return (
                    <DataList aria-label="Error causes" className="custom-rules" >
                        {
                            this.props.error.causes.map( (cause, idx) =>
                                <DataListItemRow key={""+idx} className="custom-rules-item">
                                    <DataListItemCells
                                        dataListCells={[
                                            ...
                                            <DataListCell key="main content">
                                                ...
                                                <div className="custom-rule-description">{cause.context != null ? cause.description : cause.context }</div>
                                            </DataListCell>
                                        ]}
                                    />
                                </DataListItemRow>
